defmodule TimeKeeper.ProfileController do
  ...
  def create(conn, %{"profile" => profile_params}) do
    ...
    changeset = Profile.changeset(%Profile{},
      Map.put(profile_params, "github_id", get_provider_id(conn))
    )
    IO.inspect(changeset, [pretty: true, width: 1])
  ...
  def register(conn, _params) do
    provider_urs = get_session(conn, :provider_user)
    profile = %Profile{
      email:  provider_urs["email"],
      first_name: provider_urs["name"]
                    |> String.split
                    |> List.first,
      last_name: provider_urs["name"]
                    |> String.split
                    |> List.delete_at(0)
                    |> Enum.join " "
    }
